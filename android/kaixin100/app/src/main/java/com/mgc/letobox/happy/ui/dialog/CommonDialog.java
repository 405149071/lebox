...
//        window.setBackgroundDrawableResource(android.R.color.transparent);
        ...
        if(title!=null){
            tvTitle.setText(title);
            tvTitle.setTextColor(context.getResources().getColor(R.color.mgc_sdk_lucky_dialog_red));
        ...
        if(!TextUtils.isEmpty(cancelText)){
            btcancel.setText(cancelText);
            btcancel.setHeight(BaseAppUtil.dip2px(context,32));
            btcancel.setTextColor(context.getResources().getColor(R.color.mgc_sdk_lucky_dialog_btn_left));
            btcancel.setBackground(context.getResources().getDrawable(R.drawable.lucky_dialog_btn_left));
        }
        if(!TextUtils.isEmpty(confirmText)){
            btok.setText(confirmText);
            btok.setHeight(BaseAppUtil.dip2px(context,32));
...
//
//
//    public void showIDHelpDialog(Context context, ConfirmDialogListener listener, Boolean isCanceledOnTouchOutSide){
//        dismiss();
//        this.mlistener=listener;
//        View dialogview = LayoutInflater.from(context).inflate(R.layout.id_dialog_help, null);
//        dialog = new Dialog(context,R.style.dialog_bg_style);
//        //设置view
//        dialog.setContentView(dialogview);
//        dialog.setCanceledOnTouchOutside(isCanceledOnTouchOutSide);
...
//////        window.setGravity(Gravity.BOTTOM);
////        windowparams.height = ;
//        windowparams.width = BaseAppUtil.getDeviceWidth(context)-2* BaseAppUtil.dip2px(context, 20);
////        设置背景透明,但是那个标题头还是在的，只是看不见了
//        //注意：不设置背景，就不能全屏
////        window.setBackgroundDrawableResource(android.R.color.transparent);
//
//        btok = (TextView) dialogview.findViewById(R.id.btn_ok);
//
//        btok.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
